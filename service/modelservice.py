...
class APIModelProvider(ModelService):
    ...
    def _api_call(self, endpoint, data, api_key, retries=20, retry_grace_time=10):
            ...
            while True:
                ...
                timeout=120)
                    ...
                    if(trials>max_retries):
                        temp = {'id': 'chatcmpl-8rqZ9WQDC1s21ti4alED5h515KFm0', 'object': 'chat.completion', 'created': 1707844139, 'model': 'gpt-4-0125-preview', 'choices': [{'index': 0, 'message': {'role': 'assistant', 'content': 'I cannot process this due to technical reasons.'}, 'logprobs': None, 'finish_reason': 'stop'}], 'usage': {'prompt_tokens': 22, 'completion_tokens': 14, 'total_tokens': 36}, 'system_fingerprint': 'fp_f084bcfc79'}
                        self.response = temp
                        return temp
                        break
                    print("timeout, retry")
...
class OpenAIChatModelProvider(APIModelProvider):
    ...
    def _request_format(self,
        ...
        if(genparams.system_prompt==True):
            print("use the system prompt")
            req = {
            "messages": [{"role":'system','content':'You are a very helpful assistant.'},{"content":context,"role":"user"}],
            #"echo": False,
            "max_tokens": max_token,
            #"logprobs": 1,
            "temperature": genparams.temperature,
...
class AzureOpenAIModelProvider(APIModelProvider):
    ...
    def _updateendpoint(self,):
        ...
        self.openai.api_base = "https://gpt4v222.openai.azure.com/"
        self.openai.api_version = "2023-03-15-preview"
        self.openai.api_key = os.getenv("AZURE_OPENAI_API_KEY")
        return
...
class LLama2ChatModelProvider(APIModelProvider):
    ...
    def _compute_input_token(self,context):
        

        inputs = self.tokenizer(context, return_tensors="pt")
        size1 = len(inputs['input_ids'][0])
        return size1
    def _adjust_tokens(self,context,max_tokens):
        tokens_input = self._compute_input_token(context)
        print("input token num",tokens_input)

        if(max_tokens+tokens_input>4085):
            max_tokens = 4085-tokens_input
        return max_tokens
    ...
    def getcompletion(self,
        ...
        temperature = genparams.get_dict()['temperature']
        max_tokens = genparams.max_tokens
        max_tokens = self._adjust_tokens(context,max_tokens)
        ...
        #'''
        response = predict(self._ENDPOINT, {"inputs": [prompt], "parameters": {'temperature': temperature,'max_new_tokens':max_tokens}},timeout=120)
        #'''
        '''while(1):
            try:
...
_PROVIDER_MAP = {
    ...
    "llama2chat":LLama2ChatModelProvider,
}
